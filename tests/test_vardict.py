...
vardict_single_calls = [
    [
        ...
        "filter",
        "--inputVcf",
        "tests/data/vardict/single_test.vcf",
        "--tsampleName",
        "Myeloid200-1",
        "-ad",
        "1",
        "-o",
        "tests/data/vardict/single",
    ],
]
...
vardict_matched = [
    [
        ...
        "filter",
        "--inputVcf",
        "tests/data/vardict/case_control_test.vcf",
        "--tsampleName",
        "C-C1V52M-L001-d",
        "-ad",
        "1",
        "-o",
        "tests/data/vardict/two",
    ]
]
...
def test_single(call):
    ...
    assert (
        os.path.exists("tests/data/vardict/single/single_test_STDfilter_complex.vcf")
        == True
    )
...
def test_two(call):
    ...
    assert "" in result.stdout
    assert (
        os.path.exists("tests/data/vardict/two/case_control_test_STDfilter_complex.vcf")
        == True
    )
    assert (
        os.path.exists("tests/data/vardict/two/case_control_test_STDfilter.vcf") == True
    )
    assert (
        os.path.exists("tests/data/vardict/two/case_control_test_STDfilter.txt") == True
    )
