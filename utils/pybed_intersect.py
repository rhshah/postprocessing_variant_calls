...
def annotater(maf_df, bed_df, cname):
    ...
    mafdf_dic = dict(
        mafdf_sub.set_index("Chromosome")
        .groupby(level=0)
        .apply(lambda x: x.to_dict(orient="records"))
    )
    beddf_dic = dict(
        bed_df.set_index("Chromosome")
        .groupby(level=0)
        .apply(lambda x: x.to_dict(orient="records"))
    )
    ...
    for k1, v1 in mafdf_dic.items():
        ...
        for idx, loc_maf in enumerate(v1):
            ...
            while found is False and idb <= (len(subset_bed) - 1):
                ...
                include_logic = (
                    loc_maf["Start_Position"] >= loc_bed["Start_Position"]
                ) and (loc_maf["End_Position"] < loc_bed["End_Position"])
                if include_logic:
                    # add comment to bed
                    loc_maf[cname] = "yes"
    ...
    final_annotate = combined_annotations.merge(
        maf_df,
        on=[
            "Start_Position",
            "End_Position",
