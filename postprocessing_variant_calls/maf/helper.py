...
def apply_filter_maf(pre_filter_maf,**kwargs):
    ...
    def tag_germline(mut, status,inner_kwargs):
        ...
        if 'n_vaf_fragment' in mut.index.tolist() and mut['n_ref_count_fragment'] + mut['n_alt_count_fragment'] > float(inner_kwargs['normal_TD_min']) :
            status = status + 'Germline;'
            return status
        elif (
            'common_variant' in mut['FILTER'] and
            (mut['t_ref_count_fragment'] + mut['t_alt_count_fragment']) > float(inner_kwargs['tumor_TD_min']) and
            mut['t_vaf_fragment'] > float(inner_kwargs['tumor_vaf_germline_thres'])
            ):
    ...
    def tag_below_alt_threshold(mut, status, inner_kwargs):
        if mut['t_alt_count_fragment'] < float(inner_kwargs['tier_one_alt_min']) or (mut['hotspot_whitelist'] == False and mut['t_alt_count_fragment'] < float(inner_kwargs['tier_two_alt_min'])):
            if mut['caller_t_alt_count']>= float(inner_kwargs['tier_two_alt_min']) or (mut['hotspot_whitelist'] == True and mut['caller_t_alt_count'] >= float(inner_kwargs['tier_one_alt_min'])):
    ...
    def occurrence_in_curated (mut, status, inner_kwargs):
        if mut['CURATED_n_fillout_sample_alt_detect'] >= float(inner_kwargs['min_n_curated_samples_alt_detected']):
    ...
    def occurrence_in_normal (mut, status, inner_kwargs):
        ...
        if mut['t_ref_count_fragment'] + mut['t_alt_count_fragment'] > float(inner_kwargs['tumor_TD_min']):
            if mut['CURATED_median_VAF'] != 0:
                if mut['t_vaf_fragment'] / mut['CURATED_median_VAF'] < float(inner_kwargs['tn_ratio_thres']):
                    status = status + 'TNRatio-curatedmedian;'
            
            if 'n_vaf_fragment' in mut.index.tolist():
                if mut['n_ref_count_fragment'] + mut['n_alt_count_fragment'] > float(inner_kwargs['normal_TD_min']) and mut['n_vaf_fragment'] != 0:
                    if mut['t_vaf_fragment'] / mut['n_vaf_fragment'] < float(inner_kwargs['tn_ratio_thres']):
    ...
    def in_blacklist (mut, status, inner_kwargs):
        ...
        if str(mut['Chromosome'])+"_"+ str(mut['Start_Position'])+"_"+str(mut['End_Position'])+"_"+str(mut['Reference_Allele'])+"_"+str(mut['Tumor_Seq_Allele2']) in inner_kwargs['blacklist_lst']:
...
def make_condensed_post_filter(df_post_filter):
    ...
    df_selected['D_t_depth_count_fragment']=df_selected['D_t_alt_count_fragment']+df_selected['D_t_ref_count_fragment']
    ...
    keep=['Tumor_Sample_Barcode','caller_Norm_Sample_Barcode','Matched_Norm_Sample_Barcode', 'Chromosome','Start_Position', 'Reference_Allele',	'Tumor_Seq_Allele2', 'Variant_Classification','Hugo_Symbol','HGVSp_Short','HGVSc','all_effects','dbSNP_RS','hotspot_whitelist','ExAC_AF','CallMethod', 'D_t_depth_count_fragment',	'D_t_alt_count_fragment',	'D_t_ref_count_fragment',	'D_t_vaf_fragment','n_depth_count_fragment',	'n_alt_count_fragment',	'n_ref_count_fragment',	'n_vaf_fragment']
    col=list(df_post_filter)
...
def _extract_fillout_type(df_full_fillout): # run extract fillout type function on the fillout df (will result in many mini dfs)
        ...
        df_for_genotypes = df_full_fillout[df_full_fillout['fillout_type'].isin(['CONTROL','CASE'])]
        ...
        df_control_simplex_duplex_summary_added = _find_VAFandsummary(df_control_simplex_summary_added,'duplex')
        df_all_control = _find_VAFandsummary(df_control_simplex_duplex_summary_added,'simplex_duplex')

        df_genotypes_simplex_summary_added = _find_VAFandsummary(df_for_genotypes,'simplex')
        df_genotypes_simplex_duplex_summary_added = _find_VAFandsummary(df_genotypes_simplex_summary_added,'duplex')
        df_all_genotypes = _find_VAFandsummary(df_genotypes_simplex_duplex_summary_added,'simplex_duplex')
        ...
        return df_all_curated,df_all_plasma,df_all_tumor,df_all_normals,df_all_control,df_all_genotypes
...
def _create_fillout_summary(df_fillout,alt_thres,mutation_key):
        ...
        new_columns = {col: f"{col}-{fillout_type}" for col in summary_table.columns if not col.startswith(fillout_type)}
...
def _extract_tn_genotypes(df_genotypes,tumor_samplename, normal_samplename):
    ...
    df_genotypes = df_genotypes[['Tumor_Sample_Barcode','t_alt_count_fragment', 't_ref_count_fragment','t_vaf_fragment']]
        
    df_tn_genotype = df_genotypes[df_genotypes['Tumor_Sample_Barcode']==str(tumor_samplename)]
    ...
    if str(normal_samplename) != '':
        df_n_genotype = df_genotypes[df_genotypes['Tumor_Sample_Barcode']==str(normal_samplename)] 
        ...
        df_n_genotype_final = df_n_genotype[['Matched_Norm_Sample_Barcode', 'n_alt_count_fragment', 'n_ref_count_fragment', 'n_vaf_fragment']]   
        
    df_tn_genotype_final = pd.concat([df_tn_genotype.reset_index(drop=True), df_n_genotype_final.reset_index(drop=True)], axis=1)
...
def make_pre_filtered_maf(df_annotation,df_all_curated,df_all_plasma,df_all_tumor,df_all_normals,df_all_genotypes,mutation_key,tumor_detect_alt_thres,curated_detect_alt_thres,plasma_detect_alt_thres,control_detect_alt_thres,tumor_samplename,normal_samplename):
    tumor_summary_table = _create_fillout_summary(df_all_tumor,tumor_detect_alt_thres,mutation_key)
    genotypes_summary_table = _create_fillout_summary(df_all_genotypes,control_detect_alt_thres,mutation_key)
    ...
    df_tn_genotype_final = _extract_tn_genotypes(df_all_genotypes,tumor_samplename,normal_samplename)
    
    df_tn_genotype_formerge = df_tn_genotype_final.drop(columns=['Tumor_Sample_Barcode'])
    

    df_anno_with_genotypes = pd.concat([df_annotation.reset_index(drop=True), df_tn_genotype_formerge.reset_index(drop=True)], axis=1)
    
    df_anno_with_genotypes.index = df_annotation.index
    df_pre_filter = df_anno_with_genotypes.merge(tumor_summary_table,left_index=True,right_index=True).merge(normal_summary_table,left_index=True,right_index=True).merge(curated_summary_table,left_index=True,right_index=True).merge(plasma_summary_table,left_index=True,right_index=True)
