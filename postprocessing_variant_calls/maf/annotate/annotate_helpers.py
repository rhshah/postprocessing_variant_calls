...
def maf_bed_annotate(maf, bed, cname, outputFile):
    ...
    bed_df = pd.read_csv(
        bed, sep="\t", skiprows=skip, low_memory=False, header=None, names=bed_names
    )
    ...
    # write to csv
    typer.secho(
        f"Writing out maf file to the following location: {outputFile}.csv".format(
...
def read_bed(bed):
    ...
    # remove the string "chr"
    bed_df["Chromosome"] = bed_df["Chromosome"].astype(str).str.replace("chr", "")
    return bed_df
